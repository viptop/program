    ...
    var cart = {
        ...
        bindCheckboxClick: function () {
            ...
            $('input[type=checkbox]', $('.shop_title')).click(function () {
                ...
                $('input[type=radio]', $('.cart_item', p)).prop('checked', $(this).prop('checked'));
        ...
        batchPromotionUpdate: function(p, checked) {
            var batch_count = 1; console.log(p);
        ...
        deleteItem:function (jq) {
            ...
            $.dialogBox('确定要删除吗？','',function(){
                $.post('/homes/delcarts',param,function(){
                    item.remove();$.simpleBox('删除成功');
                });
            },function(){
                em.removeClass('on');
            });
        ...
        collect:function(){
            ...
            $.dialogBox('您确定收藏选定的么？','',function(){
                $.post('add_to_wishlist',param,function(){
                    $.simpleBox('收藏成功');
...
//                                console.log(data[i])
        ...
        bindCheckout: function() {
            $('#do_checkout').click(function(){
                if(hasChecked == 0)
                    return false;
                var ids=$('input[name="items"]:checked').val();
                var uids=$('input[name="stpid"]').val();
                var asided=$('input[name="asided"]').val();
                var buy_num=$('input[name="buy_num"]').val();
                if(!ids){ $.simpleBox('未选择地址');return false; }
                var data = {'id':ids,'stid':uids,'asid':asided,'bnum':buy_num};
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if(data.errorCode != 0) {
                            ...
                            // go to checkout
                            $.simpleBox('購買成功');alert('积分更新成功');history.back();
                        }
                    }
                });
            });
...
function charse(d){
    var sd=$('textarea[name="textdid"]').val();
    if(!sd){
        alert('请填写完整');return false;
    }
    $('#forem').submit();//history.back(-2);
}
