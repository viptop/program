    ...
    render() {
        ...
        return (
            <div className={'wechat_users'} style={{ height: this.props.heihgt }}>
                <Menu style={{ width: 180 }} mode="inline" selectedKeys={selectedKeys} onSelect={this.handleClick.bind(this)}>
                    {
                        this.props.users.map((u) => {
                            if (u.hasUnread) {
                                return (
                                    <Menu.Item key={'' + u.Uin}><Badge dot count={0}><img src={u.userAvatar} /></Badge>{u.NickName}</Menu.Item>
                                );
                            } else {
                                return (
                                    <Menu.Item key={'' + u.Uin}><img src={u.userAvatar} />{u.NickName}</Menu.Item>
                                );
                            }
                        })
                    }
